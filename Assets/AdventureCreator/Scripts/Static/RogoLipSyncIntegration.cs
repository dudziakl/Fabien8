...
namespace AC
{
	...
	public class RogoLipSyncIntegration : ScriptableObject
	{
		...
		public static void Play (Char speaker, int lineNumber, string language)
		{
			if (speaker == null)
			{
				return;
			...
			#if RogoLipSyncIsPresent
			if (lineNumber > -1 && speaker != null && KickStarter.speechManager.searchAudioFiles)
			{
				...
				if (KickStarter.speechManager.autoNameSpeechFiles)
				{
					...
					string speakerName = KickStarter.speechManager.GetLineFilename (lineNumber);
				...
				{
					Object _object = KickStarter.runtimeLanguages.GetLineCustomLipsyncFile (lineNumber, Options.GetLanguage ());
